    ...
    <div class="max-w-3xl mt-20 mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <form action="{{ route('facturas.update', $factura) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" id="cita_id" name="cita_id" required>
                    @foreach ($citas as $cita)
                        <option value="{{ $cita->id }}" {{ old('cita_id', $factura->cita_id) == $cita->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" id="monto_total" name="monto_total" value="{{ old('monto_total', $factura->monto_total) }}" required>
            </div>
