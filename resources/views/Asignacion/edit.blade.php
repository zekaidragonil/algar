        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('asignacion.update', $asignacion->id) }}" method="POST" class="bg-white p-6 rounded-md shadow-md">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select id="doctor_id" name="doctor_id" class="border-gray-300 rounded w-full p-2 bg-white" required>
                                ...
                                @foreach ($doctores as $doctor)
                                    <option value="{{ $doctor->id }}" {{ $doctor->id == $asignacion->doctor_id ? 'selected' : '' }}>
                                        Dr: {{ $doctor->name }} - especialidad: {{ $doctor->especilidad }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="cita_id" name="asignado" class="border-gray-300 rounded w-full p-2 bg-white" required>
                                ...
                                @foreach ($citas as $cita)
                                    <option value="{{ $cita->id }}" {{ $cita->id == $asignacion->asignado ? 'selected' : '' }}>
                                        Paciente: {{ $cita->paciente->name }} - Especialidad: {{ $cita->especialidad }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
