<x-app-layout>
    <div class="max-w-3xl mt-20 mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <h2 class="text-2xl font-semibold mb-6 text-gray-800">Editar Receta</h2>
        ...
        <form action="{{ route('recetas.update', $receta) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" id="cita_id" name="cita_id" required>
                    @foreach ($citas as $cita)
                        <option value="{{ $cita->id }}" {{ old('cita_id', $receta->cita_id) == $cita->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="text" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" id="medicamento" name="medicamento" value="{{ old('medicamento', $receta->medicamento) }}" required>
            </div>
    
            <div class="mb-4">
                <label for="dosis" class="block text-sm font-medium text-gray-700">Dosis</label>
                <input type="text" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" id="dosis" name="dosis" value="{{ old('dosis', $receta->dosis) }}" required>
            </div>
    
            <div class="mb-4">
                <label for="frecuencia" class="block text-sm font-medium text-gray-700">Frecuencia</label>
                <input type="text" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" id="frecuencia" name="frecuencia" value="{{ old('frecuencia', $receta->frecuencia) }}" required>
            </div>
    
            <div class="mb-4">
                <label for="duracion" class="block text-sm font-medium text-gray-700">Duración</label>
                <input type="text" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" id="duracion" name="duracion" value="{{ old('duracion', $receta->duracion) }}" required>
            </div>
